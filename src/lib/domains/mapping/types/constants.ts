...
const HEXMAP_BASE_RADIUS = 1;
const HEXMAP_RADIUS_RATIO = 3;
...
assert(HEXMAP_RADIUS.XS === HEXMAP_BASE_RADIUS * HEXMAP_RADIUS_RATIO ** 0);
assert(HEXMAP_RADIUS.S === HEXMAP_BASE_RADIUS * HEXMAP_RADIUS_RATIO ** 1);
assert(HEXMAP_RADIUS.M === HEXMAP_BASE_RADIUS * HEXMAP_RADIUS_RATIO ** 2);
assert(HEXMAP_RADIUS.L === HEXMAP_BASE_RADIUS * HEXMAP_RADIUS_RATIO ** 3);
assert(HEXMAP_RADIUS.XL === HEXMAP_BASE_RADIUS * HEXMAP_RADIUS_RATIO ** 4);

export const MAP_SIZE = {
  XS: {
    radius: HEXMAP_RADIUS.XS,
    baseSize: HEX_SIZE.XL,
    count: 7 as const satisfies 7,
    box: 1960 as const satisfies 1960,
  },
  S: {
    radius: HEXMAP_RADIUS.S,
    baseSize: HEX_SIZE.L,
    count: 49 as const satisfies 49,
    box: 2223 as const satisfies 2223,
  },
  M: {
    radius: HEXMAP_RADIUS.M,
    baseSize: HEX_SIZE.M,
    count: 343 as const satisfies 343,
    box: 2520 as const satisfies 2520,
  },
  L: {
    radius: HEXMAP_RADIUS.L,
    baseSize: HEX_SIZE.S,
    count: 2401 as const satisfies 2401,
    box: 2808 as const satisfies 2808,
  },
  XL: {
    radius: HEXMAP_RADIUS.XL,
...
assert(MAP_SIZE.XS.box === 2 * HEXMAP_RADIUS.XS * HEX_SIZE.XL);
assert(MAP_SIZE.S.box === 2 * HEXMAP_RADIUS.S * HEX_SIZE.L);
assert(MAP_SIZE.M.box === 2 * HEXMAP_RADIUS.M * HEX_SIZE.M);
assert(MAP_SIZE.L.box === 2 * HEXMAP_RADIUS.L * HEX_SIZE.S);
assert(MAP_SIZE.XL.box === 2 * HEXMAP_RADIUS.XL * HEX_SIZE.XS);
