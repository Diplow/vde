...
describe("Navigation Handler", () => {
  ...
  describe("updateURL", () => {
    test("updates URL when router is available", () => {
      const handler = createNavigationHandler(config);

      handler.updateURL("456", ["3", "4"]);

      expect(mockRouter.push).toHaveBeenCalledWith(
        "/map/456?expandedItems=3%2C4",
      );
    });
    ...
    test("handles missing router gracefully", () => {
      ...
      expect(() => handler.updateURL("456", ["3", "4"])).not.toThrow();
      expect(mockRouter.push).not.toHaveBeenCalled();
    });
    ...
    test("handles empty expanded items", () => {
      ...
      handler.updateURL("456", []);

      expect(mockRouter.push).toHaveBeenCalledWith("/map/456");
    });
  });
  ...
  describe("syncURLWithState", () => {
    test("syncs URL with current state", () => {
      const handler = createNavigationHandler(config);

      handler.syncURLWithState();
    ...
    test("handles missing current center", () => {
    ...
    test("handles missing center item", () => {
  ...
  describe("getMapContext", () => {
    ...
    test("filters empty expanded items", () => {
      const handler = createNavigationHandler({
        ...config,
        searchParams: new URLSearchParams("expandedItems=1,,3,"),
      });
  ...
  describe("testing factory", () => {
    test("createNavigationHandlerForTesting works with mocked dependencies", async () => {
      ...
      const mockTestPathname = "/map/456";
      ...
      expect(mockPush).toHaveBeenCalledWith("/map/123?expandedItems=1%2C2");

      const context = handler.getMapContext();
      expect(context.pathname).toBe("/map/456");
      expect(context.searchParams.get("test")).toBe("value");
    });
  ...
  describe("handler creation", () => {
    test("returns all expected methods", () => {
      ...
      expect(typeof handler.updateURL).toBe("function");
      expect(typeof handler.prefetchForNavigation).toBe("function");
      expect(typeof handler.syncURLWithState).toBe("function");
