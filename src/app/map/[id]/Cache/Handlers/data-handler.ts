...
// Factory function for creating with server service hook
export function createDataHandlerWithServerService(
  dispatch: React.Dispatch<CacheAction>,
  state: CacheState,
  serviceConfig?: Parameters<typeof useServerService>[0],
) {
  const serverService = useServerService(serviceConfig);
