...
async function buildMap(
  depth = 1,
  mapName = `Map (Depth ${depth})`,
  description: string | null = "Auto-generated map",
) {
...
async function addCenterItem(
  ...
  try {
    await caller.map.addItem({
      ...
      coords: coord,
      title: "Center",
      descr: `Center at ${CoordSystem.createId(coord)}`,
    });
  ...
  for (const childCoord of childCoords) {
    try {
      ...
      await caller.map.addItem({
        ...
        descr: `Item at ${CoordSystem.createId(childCoord)}`,
...
async function main(): Promise<void> {
  ...
  if (!mapName) mapName = `Map (Depth ${depth})`;
  if (!description)
    description = `Auto-generated map with ${depth} levels of depth`;
  ...
  try {
    const map = await buildMap(depth, mapName, description);
